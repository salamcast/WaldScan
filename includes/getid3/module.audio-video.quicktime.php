...
class getid3_quicktime extends getid3_handler {
	...
	function Analyze() {
		...
		$info ['quicktime'] ['controller'] = 'standard'; // may be overridden if
		                                                 // 'ctyp' atom is present
		...
		while ( $offset < $info ['avdataend'] ) {
			...
			if ($atomsize == 0) {
				// Furthermore, for historical reasons the list of atoms is
				// optionally
				// terminated by a 32-bit integer set to 0. If you are writing a
				// program
				// to read user data atoms, you should allow for the terminating
				// 0.
				break;
			}
			switch ($atomname) {
				...
				case 'wide' : // 64-bit expansion placeholder atom
				              // 'free', 'skip' and 'wide' are just padding,
				              // contains
	...
	function QuicktimeParseAtom($atomname, $atomsize, $atom_data, $baseoffset, &$atomHierarchy, $ParseAllPossibleAtoms) {
		...
		switch ($atomname) {
			...
			case 'ilst' : // Item LiST container atom
				...
				// some "ilst" atoms contain data atoms that have a numeric
				// name, and the data is far more accessible if the returned
				// array is compacted
			...
			case '----' : // itunes specific
				if ($atom_parent == 'udta') {
					...
					if (substr ( $atom_data, 2, 2 ) == "\x10\xB5") {
						...
						// Each $atom_data has 2 bytes of datasize, plus 0x10B5,
						// then data
			...
			case 'ptv ' : // Print To Video - defines a movie's full screen mode
				...
				$atom_structure ['reserved_1'] = getid3_lib::BigEndian2Int ( substr ( $atom_data, 2, 2 ) ); // hardcoded:
				                                                                                            // 0x0000
				$atom_structure ['reserved_2'] = getid3_lib::BigEndian2Int ( substr ( $atom_data, 4, 2 ) ); // hardcoded:
				                                                                                            // 0x0000
						...
						case "\x00\x00\x00\x00" :
							...
							switch ($atom_structure ['sample_description_table'] [$i] ['data_format']) {
								...
								case 'mp4v' :
									...
									// $info['warning'][] = 'This version of
									// getID3() ['.$this->getid3->version().']
									// does not fully support MPEG-4 audio/video
									// streams'; // 2011-02-18: why am I warning
									// about this again? What's not supported?
									break;
			...
			case 'stts' : // Sample Table Time-to-Sample atom
				...
				for($i = 0; $i < $atom_structure ['number_entries']; $i ++) {
					...
					// // some atoms have durations of "1" giving a very large
					// framerate, which probably is not right
					// $info['video']['frame_rate'] =
					// max($info['video']['frame_rate'], $stts_new_framerate);
					// }
					// }
					//
				...
				// $sttsFramesTotal = 0;
				// $sttsSecondsTotal = 0;
				// foreach ($FrameRateCalculatorArray as $frames_per_second =>
				// $frame_count) {
				...
				// if (($sttsFramesTotal / $sttsSecondsTotal) >
				// $info['video']['frame_rate']) {
				// $info['video']['frame_rate'] = $sttsFramesTotal /
				// $sttsSecondsTotal;
				// }
				// }
				break;
			...
			case 'co64' : // Chunk Offset 64-bit (version of "stco" that supports >
			              // 2GB files)
				if ($ParseAllPossibleAtoms) {
					$atom_structure ['version'] = getid3_lib::BigEndian2Int ( substr ( $atom_data, 0, 1 ) );
					$atom_structure ['flags_raw'] = getid3_lib::BigEndian2Int ( substr ( $atom_data, 1, 3 ) ); // hardcoded:
			...
			case 'pnot' : // Preview atom
				$atom_structure ['modification_date'] = getid3_lib::BigEndian2Int ( substr ( $atom_data, 0, 4 ) ); // "standard
				                                                                                                   // Macintosh
				                                                                                                   // format"
				$atom_structure ['version_number'] = getid3_lib::BigEndian2Int ( substr ( $atom_data, 4, 2 ) ); // hardcoded:
				                                                                                                // 0x00
				$atom_structure ['atom_type'] = substr ( $atom_data, 6, 4 ); // usually:
				                                                             // 'PICT'
				$atom_structure ['atom_index'] = getid3_lib::BigEndian2Int ( substr ( $atom_data, 10, 2 ) ); // usually:
				                                                                                             // 0x01
			...
			case 'crgn' : // Clipping ReGioN atom
				$atom_structure ['region_size'] = getid3_lib::BigEndian2Int ( substr ( $atom_data, 0, 2 ) ); // The
				                                                                                             // Region
				                                                                                             // size,
				                                                                                             // Region
				                                                                                             // boundary
				                                                                                             // box,
				$atom_structure ['boundary_box'] = getid3_lib::BigEndian2Int ( substr ( $atom_data, 2, 8 ) ); // and
				                                                                                              // Clipping
				                                                                                              // region
				                                                                                              // data
				                                                                                              // fields
				$atom_structure ['clipping_data'] = substr ( $atom_data, 10 ); // constitute
				                                                               // a
				                                                               // QuickDraw
				                                                               // region.
				break;
			...
			case 'ctab' : // Color TABle atom
				$atom_structure ['color_table_seed'] = getid3_lib::BigEndian2Int ( substr ( $atom_data, 0, 4 ) ); // hardcoded:
				                                                                                                  // 0x00000000
				$atom_structure ['color_table_flags'] = getid3_lib::BigEndian2Int ( substr ( $atom_data, 4, 2 ) ); // hardcoded:
				                                                                                                   // 0x8000
			...
			case 'iods' : // Initial Object DeScriptor atom
				...
				$atom_structure ['num_iods_tracks'] = ($atom_structure ['length'] - 7) / 6; // 6
				                                                                            // bytes
				                                                                            // would
				                                                                            // only
				                                                                            // be
			...
			case 'skip' : // SKIP atom
			case 'wide' : // 64-bit expansion placeholder atom
			              // 'mdat' data is too big to deal with, contains no
			              // useful
			              // metadata
			              // 'free', 'skip' and 'wide' are just padding, contains
			              // no
			              // useful data at all
			              
				// When writing QuickTime files, it is sometimes necessary to
			              // update an atom's size.
			              // It is impossible to update a 32-bit atom to a 64-bit
			              // atom since the 32-bit atom
			              // is only 8 bytes in size, and the 64-bit atom requires
			              // 16 bytes. Therefore, QuickTime
			              // puts an 8-byte placeholder atom before any atoms it
			              // may
			              // have to update the size of.
			              // In this way, if the atom needs to be converted from a
			              // 32-bit to a 64-bit atom, the
			              // placeholder atom can be overwritten to obtain the
			              // necessary 8 extra bytes.
			              // The placeholder atom has a type of
			...
			case 'imgt' : // IMaGe Track reference (kQTVRImageTrackRefType) (seen
			              // on
			              // QTVR)
			...
			// Observed-but-not-handled atom types are just listed here to
			// prevent warnings being generated
			case 'FXTC' : // Something to do with Adobe After Effects (?)
			case 'PrmA' :
			case 'code' :
			case 'FIEL' : // this is NOT "fiel" (Field Ordering) as describe here:
			              // http://developer.apple.com/documentation/QuickTime/QTFF/QTFFChap3/chapter_4_section_2.html
			case 'tapt' : // TrackApertureModeDimensionsAID -
			              // http://developer.apple.com/documentation/QuickTime/Reference/QT7-1_Update_Reference/Constants/Constants.html
			              // tapt seems to be used to compute the video size
			              // [http://www.getid3.org/phpBB3/viewtopic.php?t=838]
			              // *
			...
			case 'NCDT' :
				...
				// Nikon-specific QuickTime tags found in the NCDT atom of MOV
				// videos from some Nikon cameras such as the Coolpix S8000 and
				// D5100
			...
			case 'meta' : // METAdata atom
			              // some kind of metacontainer, may contain a big data
			              // dump
			              // such as:
			              // mdta keys mdtacom.apple.quicktime.make
			              // (mdtacom.apple.quicktime.creationdate
			              // ,mdtacom.apple.quicktime.location.ISO6709
			              // $mdtacom.apple.quicktime.software
			              // !mdtacom.apple.quicktime.model ilst data DEApple 0
			              // (data
			              // DE2011-05-11T17:54:04+0200 2 *data
			              // DE+52.4936+013.3897+040.247/ data DE4.3.1 data
			              // DEiPhone 4
				...
				// $this->QuicktimeParseContainerAtom($atom_data, $baseoffset +
				// 8, $atomHierarchy, $ParseAllPossibleAtoms);
				break;
			
			case 'data' : // metaDATA atom
			              // seems to be 2 bytes language code (ASCII), 2 bytes
			              // unknown (set to 0x10B5 in sample I have), remainder is
			              // useful
			              // data
	...
	function QuicktimeParseContainerAtom($atom_data, $baseoffset, &$atomHierarchy, $ParseAllPossibleAtoms) {
		// echo 'QuicktimeParseContainerAtom('.substr($atom_data, 4, 4).') @
		// '.$baseoffset.'<br><br>';
	...
	function QuicktimeParseNikonNCTG($atom_data) {
		// http://www.sno.phy.queensu.ca/~phil/exiftool/TagNames/Nikon.html#NCTG
		// Nikon-specific QuickTime tags found in the NCDT atom of MOV videos
		// from some Nikon cameras such as the Coolpix S8000 and D5100
		...
		// 0x0005 = float (size field *= 8-byte), values are stored aaaabbbb
		// where value is aaaa/bbbb; possibly multiple sets of values appended
		// together
		// 0x0007 = bytes (size field *= 1-byte), values are stored as ??????
		// 0x0008 = ????? (size field *= 2-byte), values are stored as ??????
		// * 2 bytes data size field
		// * ? bytes data (string data may be null-padded; datestamp fields are
		// in the format "2011:05:25 20:24:15")
		...
		while ( $offset < $datalength ) {
			// echo getid3_lib::PrintHexBytes(substr($atom_data, $offset,
			// 4)).'<br>';
			...
			switch ($data_size_type) {
				...
				case 0x0002 : // 0x0002 = char (size field *= 1-byte)
					...
					$data = rtrim ( $data, "\x00" );
					break;
				case 0x0003 : // 0x0003 = DWORD+ (size field *= 2-byte), values are
				              // stored CDAB
					$data = '';
					...
					$offset += ($data_size * 2);
					break;
				case 0x0004 : // 0x0004 = QWORD+ (size field *= 4-byte), values are
				              // stored EFGHABCD
					$data = '';
					for($i = $data_size - 1; $i >= 0; $i --) {
					...
					$offset += ($data_size * 4);
					break;
				case 0x0005 : // 0x0005 = float (size field *= 8-byte), values are
				              // stored aaaabbbb where value is aaaa/bbbb; possibly
				              // multiple
				              // sets of values appended together
					$data = array ();
					...
					break;
				case 0x0007 : // 0x0007 = bytes (size field *= 1-byte), values are
				              // stored as ??????
					$data = substr ( $atom_data, $offset, $data_size * 1 );
					$offset += ($data_size * 1);
					break;
				case 0x0008 : // 0x0008 = ????? (size field *= 2-byte), values are
				              // stored as ??????
			...
			switch ($record_type) {
				...
				case 0x02000024 : // WorldTime
				                  ...
				                  // timezone is stored as offset from GMT in
				                  // minutes
	...
	function Pascal2String($pascalstring) {
		// Pascal strings have 1 unsigned byte at the beginning saying how many
		// chars (1-255) are in the string
